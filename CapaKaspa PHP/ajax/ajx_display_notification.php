...
if ($start == 0 && $numNotifications == 0)
{
	...
	while($tmpNotification = mysqli_fetch_array($tmpNotifications, MYSQLI_ASSOC))
	{
		...
		echo("
				<div class='activity' style='margin-bottom: 0px; padding-top: 5px; font-size: 12px;' id='notif".$tmpNotification['notifType'].$tmpNotification['notifID']."' onmouseover=\"this.style.cursor='pointer';\" ".$link.">
					<div class='leftbar'>
						<img src='".getPicturePath($tmpNotification['socialNetwork'], $tmpNotification['socialID'])."' width='40' height='40' border='0'/>
					</div>
					<div class='title'>
