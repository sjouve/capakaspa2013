...
function displayPlayersRanking(start, player, critCtry, critType)
{
	if (start == 0)
		document.getElementById("players"+start).innerHTML="<img src='images/ajaxloader.gif'/>";
	document.getElementById("players"+start).style.display = "block";
	...
	xmlhttp.open("GET","ajax/ajx_display_ranking.php?start="+start+"&player="+player+"&cc="+critCtry+"&tp="+critType,true);
	xmlhttp.send();
}